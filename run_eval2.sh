...
# 指定 pipeline 方法: rag, coa, vanilla, direct
PIPELINE_METHOD="rag"  
...
if [ -z "$WEAVE_PROJECT" ]; then
    python server.py -m $PIPELINE_METHOD -p $SERVER_PORT > server_$SERVER_PORT.log 2>&1 &
else
    python server.py -m $PIPELINE_METHOD -w $WEAVE_PROJECT -p $SERVER_PORT > server_$SERVER_PORT.log 2>&1 &
fi
